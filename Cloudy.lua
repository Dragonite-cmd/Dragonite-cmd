...
local libs = {
	{
		['name'] = "HashLib",
		['url'] = "https://raw.githubusercontent.com/cloudyExecutor/webb/refs/heads/main/hashes/hash"
	},
	{
		['name'] = "lz4",
		['url'] = "https://raw.githubusercontent.com/cloudyExecutor/webb/refs/heads/main/hashes/lz4"
	},
	{
		['name'] = "DrawingLib",
...
function Bridge:SyncFiles()
	...
	if not success then
		print("[Cloudy] Server connection loss: Could not sync virtual files from client to external. Server was closed or it is being overloaded.")
		return
	end
...
function Xeno.request(options)
	...
	options.Headers["User-Agent"] = options.Headers["User-Agent"] or "cloudy/RobloxApp/" .. tostring(Xeno.about._version)
...
function Xeno.setscriptable(instance, property, isScriptable)
    if property == "size_xml" then
        if isScriptable then

            scriptableProperties[instance] = scriptableProperties[instance] or {}
            scriptableProperties[instance][property] = true
            return false 
        end
    end
end
...
function Xeno.isscriptable(instance, property)
...
function Xeno.setreadonly(object, readonly)
...
imagine getting cracked by a free decompiler
...
function Xeno.debug.getproto(f, index, mock)
...
function Xeno.debug.getupvalue(f, index)
...
-- MR. DEFAULT
function Xeno.debug.getprotos(f)
    local protos_TBL = {}
    spawn(function()
        local i = 1
        while true do
...
function Xeno.sethiddenproperty(obj, property, value)
...
function Xeno.gethiddenproperty(obj, property)
    if not obj or type(property) ~= "string" then
...
-- WEBSOCKET
Xeno.WebSocket = Xeno.WebSocket or {}
